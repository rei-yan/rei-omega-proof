...
def runtime_git_commit_sha(source_path: Path) -> str | None:
    """Best-effort Git commit provenance; returns None outside a Git worktree."""
    try:
        proc = subprocess.run(
...
class ShadowLoop:
    ...
    def cycle(self) -> dict[str, Any]:
        ...
        started_at = utc_now()
        runtime_source_sha256 = file_digest(Path(__file__).resolve())
        runtime_git_commit = runtime_git_commit_sha(Path(__file__).resolve())
        ...
        if passed_proposals == 0 and model_errors:
            ...
            if synthesis.get("runtime_fallback"):
                ...
                try:
                    ...
                    review = self.failed_review(model_errors)
        try:
            _review_score = float(review.get("score", -1.0))
        except (TypeError, ValueError):
            _review_score = -1.0
        if not 0.0 <= _review_score <= 1.0:
            model_errors.append("review_score_out_of_range")
        ...
        result = {
            ...
            "git_commit_sha": runtime_git_commit,
            "score_schema_min": REVIEW_SCHEMA["properties"]["score"].get("minimum"),
            "score_schema_max": REVIEW_SCHEMA["properties"]["score"].get("maximum"),
            "resilience_layer": RESILIENCE_LAYER,
