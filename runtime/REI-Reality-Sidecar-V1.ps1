  ...
  foreach ($file in @(Get-ChildItem -LiteralPath $Inbox -Filter '*.json' -File -ErrorAction SilentlyContinue | Sort-Object Name)) {
    ...
    if (-not $reason) {
      foreach ($field in $RequiredFields) {
        if (-not (Has-Property $item $field) -or [string]::IsNullOrWhiteSpace([string]$item.$field)) {
          $reason = "MISSING_OR_BLANK:$field"
          break
        }
      }
    }
    ...
    if ($reason) {
      $invalid++
      $rejections += [pscustomobject][ordered]@{
        file=$file.Name; reason=$reason; rejected_at_utc=[DateTime]::UtcNow.ToString('o')
      }
      continue
    }
    ...
    if ($byId.ContainsKey($id)) {
      ...
      $rejections += [pscustomobject][ordered]@{
        file=$file.Name
        evidence_id=$id
        reason='EVIDENCE_ID_HASH_CONFLICT'
        observed_sha256=$sha
        existing_sha256=[string]$byId[$id]
