...
EXPECTED_SCHEDULE_SHA256 = "56263abd9fb4cf4624c98669e68d451987987802641d501d5943d71c4362cbb2"
...
def primitive_candidates():
    ...
    Constructor families are not claimed to be invented by REI. The induced
    parameterized primitive is new relative to the frozen scaffold.
    """
    out = []
    for i in range(69):
        w = 0.60 + i * 0.05
        out.append(("osc", {"w": round(w, 2)}, [
    ...
    for i in range(49):
        tau = -1.20 + i * 0.05
        out.append(("hinge", {"tau": round(tau, 2)}, [
            (f"hinge@{tau:.2f}", lambda x, tau=tau: max(0.0, x - tau))
        ]))
        out.append(("cusp", {"tau": round(tau, 2)}, [
    ...
    for i in range(33):
        ...
        for scale in (0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.80, 1.00):
            out.append(("rbf", {"c": round(center, 2), "s": scale}, [
                (f"rbf({center:.2f},{scale:.2f})",
                 lambda x, center=center, scale=scale: math.exp(-((x - center) / scale) ** 2))
            ]))
...
def evaluate(name):
    ...
    scaffold_adequate = base_held <= HELDOUT_LIMIT and base_challenge <= CHALLENGE_LIMIT
    ...
    for kind, params, extra in primitive_candidates():
        ...
        # The wider challenge set is intentionally absent from candidate ranking.
        ranked.append((
            round(held_error, 12), len(extra), kind,
            json.dumps(params, sort_keys=True), params, features, beta
        ))
    ...
    def finalize(row):
        ...
        residuals = [abs(y - predict(features, beta, x)) for x, y in zip(CHALLENGE, challenge_y)]
        ...
        return {
            "kind": kind, "params": params, "features": features, "beta": beta,
            "held": held_error, "challenge": challenge_error,
            "max_residual": max_residual, "falsify_x": falsify_x,
            "certified": certified,
        }

    b, s = finalize(best), finalize(second)
    disagreements = [
        abs(predict(b["features"], b["beta"], x) - predict(s["features"], s["beta"], x))
        for x in MEASURE_GRID
    ]
    measure_x = MEASURE_GRID[disagreements.index(max(disagreements))]
    return {
        "name": name, "base_held": base_held, "base_challenge": base_challenge,
        "scaffold_adequate": scaffold_adequate, "best": b, "second": s,
        "measure_x": measure_x, "max_disagreement": max(disagreements),
    }
