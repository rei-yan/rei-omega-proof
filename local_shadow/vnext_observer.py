...
def read_jsonl(path: Path) -> list[dict[str, Any]]:
    """Read an observer ledger fail-closed. Never silently drop malformed history."""
    if not path.exists():
        return []
    ...
    try:
        ...
        raise RuntimeError(f"cannot read observer ledger {path}: {exc}") from exc
    for index, line in enumerate(lines, start=1):
        ...
        try:
            ...
            raise RuntimeError(f"malformed observer ledger {path}:{index}: {exc}") from exc
        if not isinstance(value, dict):
            raise RuntimeError(f"non-object observer ledger row {path}:{index}")
        out.append(value)
    return out
...
def append_jsonl_unique(path: Path, payload: dict[str, Any], key: str = "cycle_id") -> None:
    existing = read_jsonl(path)
    marker = str(payload.get(key, ""))
    if marker and any(str(item.get(key, "")) == marker for item in existing):
        return
...
def lineage_summary(evidence: list[dict[str, Any]]) -> dict[str, Any]:
    if not evidence:
        return {
            ...
            "mean_declared_independence": 0.0,
            "lineage_score": 0.0,
            "lineage_groups": [],
        }
    ...
    return {
        ...
        "mean_declared_independence": round(mean_ind, 6),
        "lineage_score": round(score, 6),
        "lineage_groups": [
            {"provenance": key, "count": len(items), "roles": sorted({item["role"] for item in items})}
...
def recurrence_summary(signals: list[str], prior: list[dict[str, Any]]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for row in prior:
        for item in row.get("failures", []) if isinstance(row.get("failures"), list) else []:
            if isinstance(item, dict) and item.get("fingerprint"):
                prior_counts[str(item["fingerprint"])] += 1
    ...
    return {
        ...
        "recurrent_failure_present": max_recurrence >= 2,
    }, failures
...
def calibration_assertions(cycle: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for wrapper in cycle.get("proposals", []) if isinstance(cycle.get("proposals"), list) else []:
        ...
        try:
            ...
            uncertainty = 1.0
        assertions.append({
            "source": str(wrapper.get("role", "unknown")),
    ...
    if "score" in review:
        assertions.append({
            "source": "internal_review_score",
            "stated_confidence": review.get("score"),
            "outcome_status": "NOT_A_PROBABILITY_FORECAST",
            "calibration_eligible": False,
...
def prospective_seal(cycle: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(explicit, str) or not explicit.strip():
        return {
            "status": "NO_EXPLICIT_PROSPECTIVE_PREDICTION",
            "externally_witnessed": False,
            "timestamp_independence_verified": False,
            "promotion_effect": "NONE",
        }
    payload = {
        "prediction": explicit.strip()[:1500],
        "cycle_id": cycle.get("cycle_id"),
        "sealed_at": utc_now(),
        "source_audit_hash": cycle.get("audit_hash"),
    }
    return {
        "status": "SEALED_INTERNAL_ONLY",
...
def build_observer(cycle: dict[str, Any], recurrence_ledger: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    advisory_conditions = {
        "base_shadow_gate_passed": base_gate,
    ...
    observer = {
        ...
        "canonical_write_permission": CANONICAL_WRITE_PERMISSION,
        "cycle_id": cycle.get("cycle_id"),
        "source_cycle_audit_hash": cycle.get("audit_hash"),
        "generated_at": utc_now(),
        "lineage": lineage,
        "hypothesis_state": {
            "policy": "maintain_multiple_hypotheses_until_evidence_justifies_collapse",
            "quantum_claim": False,
        ...
        "boundaries": [
            ...
            "LineageScoreIsHeuristic != IndependentReplication",
        ],
    }
    recurrence_record = {
        "schema_version": 1,
        "protocol_version": PROTOCOL_VERSION,
        "cycle_id": cycle.get("cycle_id"),
        "timestamp": observer["generated_at"],
        "failures": failures,
        "canonical_write_permission": False,
    }
...
def process(home: Path) -> int:
    ...
    if not cycle:
        print("VNEXT_OBSERVER_NO_CYCLE")
        return 0
    cycle_id = str(cycle.get("cycle_id", "")).strip()
    if not cycle_id:
        print("VNEXT_OBSERVER_FAILED_CLOSED: cycle_id missing")
        return 2
    ...
    prior_without_current = [row for row in prior if str(row.get("cycle_id", "")) != cycle_id]
    observer, recurrence_record = build_observer(cycle, prior_without_current)

    atomic_json_write(output_dir / f"vnext_{cycle_id}.json", observer)
    atomic_json_write(observer_dir / "latest.json", observer)
    append_jsonl_unique(recurrence_path, recurrence_record)
    append_jsonl_unique(observer_dir / "calibration_ledger.jsonl", {
        "schema_version": 1,
        "protocol_version": PROTOCOL_VERSION,
        "cycle_id": cycle_id,
        "timestamp": observer["generated_at"],
        "assertions": observer["calibration"]["assertions"],
        "canonical_write_permission": False,
    })
    append_jsonl_unique(observer_dir / "lineage_ledger.jsonl", {
        "schema_version": 1,
        "protocol_version": PROTOCOL_VERSION,
        "cycle_id": cycle_id,
        "timestamp": observer["generated_at"],
    ...
    atomic_json_write(observer_dir / "protocol_state.json", {
        "protocol_version": PROTOCOL_VERSION,
        "observer_mode": True,
        "last_cycle_id": cycle_id,
        "last_observer_sha256": observer["observer_sha256"],
        "updated_at": observer["generated_at"],
        "canonical_write_permission": False,
    })
...
def self_test() -> int:
    sample = {
        ...
        "runtime_errors": [],
        "proposals": [
            {"role": "evidence", "proposal": {"uncertainty": 0.2, "counterarguments": ["alt"], "evidence": [
            ...
            {"role": "ood", "proposal": {"uncertainty": 0.4, "counterarguments": ["shift"], "evidence": [
                {"claim": "c3", "support": "s", "provenance": "model_inference", "independence": 0.2}
            ]}},
        ],
        "synthesis": {"claim": "primary"},
        "review": {"score": 0.8, "required_revisions": []},
    }
    ...
    print("VNEXT_OBSERVER_SELF_TEST_SUCCESS")
    return 0
...
def main() -> int:
    ...
    try:
        ...
        print(f"VNEXT_OBSERVER_FAILED_CLOSED: {exc}")
        print("Canonical write permission: FALSE")
        return 2
